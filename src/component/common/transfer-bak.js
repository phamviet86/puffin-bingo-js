...
export function Transfer({
  ...
  const [searchSourceColumnsParams, setOnSourceSearchParams] = useState({});
  const [searchTargetColumnsParams, setOnTargetSearchParams] = useState({});
  ...
  const reloadSourceData = useCallback(async () => {
    ...
    const sourceParams = { ...onSourceParams, ...searchSourceColumnsParams };
  ...
  const reloadTargetData = useCallback(async () => {
    ...
    const targetParams = { ...onTargetParams, ...searchTargetColumnsParams };
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      if (searchSourceColumns.length > 0 && sourceSearchValue.trim()) {
        const or = {};
        searchSourceColumns.forEach((key) => {
  ...
  }, [sourceSearchValue, searchSourceColumns, searchDelay]);
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      if (searchTargetColumns.length > 0 && targetSearchValue.trim()) {
        const or = {};
        searchTargetColumns.forEach((key) => {
    ...
    return () => clearTimeout(timeoutId);
  }, [targetSearchValue, searchTargetColumns, searchDelay]);
